...
router.get("/", async (req, res) => {
    try {
        const minPower = req.query.minPower ? parseFloat(req.query.minPower) : undefined;
        const minSpeed = req.query.minSpeed ? parseFloat(req.query.minSpeed) : undefined;
...
router.get("/:id", async (req, res) => {
    try {
        const motorId = parseInt(req.params.id, 10);
        if (isNaN(motorId)) {
            res.status(400).json({ error: "ID không hợp lệ" });
            return;
        }
        const motor = await prisma_1.prisma.motorLibrary.findUnique({
