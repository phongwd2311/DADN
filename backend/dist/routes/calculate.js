...
function findPowerRowBySpeed(n) {
    const validN = Number.isFinite(n) && n > 0 ? n : chainTables_1.CHAIN_REF.n01;
    return chainTables_1.CHAIN_POWER_TABLE.reduce((best, current) => {
        const bestDiff = Math.abs(best.n01 - validN);
        const currentDiff = Math.abs(current.n01 - validN);
        return currentDiff < bestDiff ? current : best;
    }, chainTables_1.CHAIN_POWER_TABLE[0]);
}
function findChainIndexByPt(pt, n) {
    const row = findPowerRowBySpeed(n);
    const index = row.values.findIndex((value) => pt <= value);
    return index === -1 ? chainTables_1.CHAIN_TABLE.length - 1 : index;
}
function chooseZ1ByRatio(ux) {
    const tableHit = standards_1.CHAIN_Z1_TABLE.find((row) => ux >= row.u_from && ux < row.u_to) ?? standards_1.CHAIN_Z1_TABLE[standards_1.CHAIN_Z1_TABLE.length - 1];
...
router.post("/", async (req, res) => {
    try {
        ...
        const allMotors = await prisma_1.prisma.motorLibrary.findMany({
        ...
        const externalRatioName = externalDriveType === "CHAIN"
            ? "ux"
        ...
        if (externalDriveType === "CHAIN") {
            ...
            for (let iteration = 1; iteration <= 5; iteration += 1) {
                const selectedChain = chainTables_1.CHAIN_TABLE[Math.min(chainIndex, chainTables_1.CHAIN_TABLE.length - 1)];
                ...
                const Kn = chainTables_1.CHAIN_REF.n01 / n3;
                const Pt = P3 * K * Kz * Kn;
                ...
                const pMax = (0, standards_1.getPmaxBySprocketSpeed)(n3);
                ...
                const contactPassed = sigmaH1 <= chainTables_1.CONTACT_STRESS_ALLOW && sigmaH2 <= chainTables_1.CONTACT_STRESS_ALLOW;
                ...
                if (chainIndex >= chainTables_1.CHAIN_TABLE.length - 1) {
                    break;
                }
                chainIndex += 1;
            }
