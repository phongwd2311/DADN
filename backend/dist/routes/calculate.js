...
const efficiency_1 = require("../constants/efficiency");
const chainTables_1 = require("../constants/chainTables");
...
router.post("/", async (req, res) => {
    try {
        const parsed = schemas_1.calculateSchema.safeParse(req.body);
        if (!parsed.success) {
            res.status(400).json({
        ...
        // Hiệu suất hệ thống
        const eta = efficiency_1.EFFICIENCY.chain * Math.pow(efficiency_1.EFFICIENCY.gear, 2) * Math.pow(efficiency_1.EFFICIENCY.bearing, 4) * efficiency_1.EFFICIENCY.coupling;
        // Công suất cần thiết
        const Pct = Ptd / eta;
        // Số vòng quay trục công tác
        const nlv = (60000 * v) / (Math.PI * D);
        // Tỷ số truyền sơ bộ và số vòng quay sơ bộ
        const usb = gearbox_1.PRELIM_UX * uh;
        const nsb = nlv * usb;
        ...
        const candidates = await prisma_1.prisma.motorLibrary.findMany({
        ...
        if (candidates.length > 0) {
            ...
            motor = await prisma_1.prisma.motorLibrary.findFirst({
        ...
        const T_dc = motor?.rated_power != null && motor.rated_speed != null
            ? (9.55 * 1000000 * motor.rated_power) / motor.rated_speed
            : null;
        const overload_ok = motor?.tk_tdn != null ? motor.tk_tdn >= tmm_t1_ratio : null;
        const selectGearRatios = (ratio, type) => {
        ...
        const P3 = Plv / (efficiency_1.EFFICIENCY.chain * efficiency_1.EFFICIENCY.bearing);
        const P2 = P3 / (efficiency_1.EFFICIENCY.gear * efficiency_1.EFFICIENCY.bearing);
        const P1 = P2 / (efficiency_1.EFFICIENCY.gear * efficiency_1.EFFICIENCY.bearing);
        const Pdc_prime = P1 / (efficiency_1.EFFICIENCY.coupling * efficiency_1.EFFICIENCY.bearing);
        ...
        // Momen xoắn
        const calcT = (p, n) => (9.55 * 1000000 * p) / n;
        ...
        const K = chainTables_1.CHAIN_USE_FACTORS.k0 * chainTables_1.CHAIN_USE_FACTORS.ka * chainTables_1.CHAIN_USE_FACTORS.kdc * chainTables_1.CHAIN_USE_FACTORS.kbt * chainTables_1.CHAIN_USE_FACTORS.kd * chainTables_1.CHAIN_USE_FACTORS.kc;
        const Kz = chainTables_1.CHAIN_REF.z01 / z1;
        const Kn = chainTables_1.CHAIN_REF.n01 / (n3 ?? chainTables_1.CHAIN_REF.n01);
        const Pt = P3 * K * Kz * Kn;
        const powerRow = chainTables_1.CHAIN_POWER_TABLE.find((row) => row.n01 === chainTables_1.CHAIN_REF.n01) ?? chainTables_1.CHAIN_POWER_TABLE[0];
        const powerIndex = powerRow.values.findIndex((value) => Pt <= value);
        const chainIndex = powerIndex === -1 ? chainTables_1.CHAIN_TABLE.length - 1 : powerIndex;
        const selectedChain = chainTables_1.CHAIN_TABLE[chainIndex];
        ...
        const a_star = 0.25 * p * (xc - 0.5 * (z1 + z2) + Math.sqrt(Math.pow(xc - 0.5 * (z1 + z2), 2) - 2 * Math.pow(diff_z / Math.PI, 2)));
        ...
        const F0 = 9.81 * chainTables_1.CHAIN_USE_FACTORS.kf * selectedChain.q * (a / 1000);
        const s = selectedChain.Q / (chainTables_1.CHAIN_USE_FACTORS.kd * Ft + F0 + Fv);
        const s_min_data = chainTables_1.SAFETY_FACTOR_TABLE.find((item) => item.p === p) || { s_min: 8.5 };
        const Fvd = 13e-7 * (n3 ?? 0) * Math.pow(p, 3) * 1;
        const calcSigmaH = (kr) => {
            const insideSqrt = (kr * (Ft * chainTables_1.CHAIN_USE_FACTORS.kd + Fvd) * chainTables_1.ELASTIC_MODULUS) / (selectedChain.A * 1);
            return 0.47 * Math.sqrt(insideSqrt);
        };
        const sigmaH1 = calcSigmaH(1.0);
        const sigmaH2 = calcSigmaH(0.8);
        const Fr = chainTables_1.CHAIN_USE_FACTORS.kx * Ft;
        const chainParams = {
            z1,
            z2,
            p,
        ...
        const chainStrength = {
            ...
            sigmaH1,
            sigmaH2,
            sigmaH_allow: chainTables_1.CONTACT_STRESS_ALLOW,
            contactPassed: sigmaH1 <= chainTables_1.CONTACT_STRESS_ALLOW && sigmaH2 <= chainTables_1.CONTACT_STRESS_ALLOW,
        };
