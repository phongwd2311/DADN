...
router.get("/:tableKey", async (req, res) => {
    try {
        ...
        if (tableKey === "P1.2") {
            ...
            const motors = await prisma_1.prisma.motorLibrary.findMany({
                where: {
        ...
        if (tableKey === "3.1") {
            ...
            let rows = (0, standards_1.buildGearboxRatioTable)(gearboxType);
        ...
        if (tableKey === "5.4") {
            ...
            if (!Number.isFinite(u)) {
                res.json({ table: "5.4", data: standards_1.CHAIN_Z1_TABLE });
                return;
            }
            const uVal = Number(u);
            const matched = standards_1.CHAIN_Z1_TABLE.find((row) => uVal >= row.u_from && uVal < row.u_to) ?? standards_1.CHAIN_Z1_TABLE[standards_1.CHAIN_Z1_TABLE.length - 1];
            ...
            res.json({
                ...
                data: standards_1.CHAIN_Z1_TABLE,
            });
            return;
        }
        if (tableKey === "5.8") {
            ...
            if (!Number.isFinite(n)) {
                res.json({ table: "5.8", data: standards_1.CHAIN_MAX_PITCH_TABLE });
                return;
            }
            const nVal = Number(n);
            res.json({
                table: "5.8",
                query: { n: nVal },
                recommended: { p_max: (0, standards_1.getPmaxBySprocketSpeed)(nVal) },
                data: standards_1.CHAIN_MAX_PITCH_TABLE,
            });
            return;
        }
        res.json({ table: tableKey, data: (0, standards_1.getStandardsTableData)(tableKey) });
    }
    catch (err) {
