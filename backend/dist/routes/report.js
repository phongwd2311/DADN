...
function parseReportBody(body) {
    ...
    const chainStrength = reportBody.result.chainStrength ?? {};
...
function buildPdfLines(preview) {
    const lines = [];
    const generatedAt = new Date(preview.generated_at).toLocaleString("vi-VN");
    const input = preview.sections.input ?? {};
    const result = preview.sections.result ?? {};
    ...
    for (const item of formulas) {
        if (!item || typeof item !== "object")
            continue;
        const id = sanitizePdfText(item.id ?? "N/A");
        const formula = sanitizePdfText(item.formula ?? "N/A");
        const substitution = sanitizePdfText(item.substitution ?? "N/A");
