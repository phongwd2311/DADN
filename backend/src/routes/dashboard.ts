...
router.get("/", async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const draftsCount = await prisma.draftSession.count({
      where: {
        user_id: userId
      },
    });

    const templatesCount = await prisma.inputTemplate.count({
      where: {
        user_id: userId
      },
    });

    const motorsCount = await prisma.motorLibrary.count();
    ...
    const recentCalculations = sessions.slice(0, 5).map((s) => {
      ...
      const motor = isObject((result as Record<string, unknown>).motor)
        ? ((result as Record<string, unknown>).motor as Record<string, unknown>)
        : {};
      ...
      return {
        ...
        pct: getNumber((result as Record<string, unknown>).Pct),
